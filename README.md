# DeepSea Jewels: Unity 매치3 게임 포트폴리오

<a href="https://littlebigcha.itch.io/undertheseamatch3" target="_blank" rel="noopener noreferrer"> itch.io에서 지금 플레이하기 (WebGL)</a>

이 프로젝트는 Unity로 개발된 기능 완성형 매치3 게임입니다. 단순한 기능 구현을 넘어, 라이브 서비스 환경에서 요구되는 확장성, 성능 최적화, 그리고 정교한 알고리즘(예: 연쇄 반응 큐) 구현에 중점을 두었습니다.
...
*   **연쇄 반응 (Chain Reaction)**: 특수 젬의 폭발이 또 다른 특수 젬을 터뜨릴 경우 모든 특수 젬이 연쇄적으로 발동합니다.
*   **다이나믹 보드**: 마스터 젬 프리팹 9종 중 6종만 랜덤으로 선택되어 매 게임마다 새로운 경험을 제공합니다.
...
4.  **데이터 기반(Prefab-Driven) 설계**
    ...
    *   **다이나믹 스폰**: `SelectActiveGems` 함수는 게임 시작 시 9개의 마스터 프리팹 중 6개만 랜덤으로 `activeGemPrefabs` 리스트에 담습니다. 모든 젬 생성 로직은 이 활성 리스트를 참조합니다.
